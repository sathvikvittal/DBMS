...
-- DROP TABLE IF EXISTS ORDER_PER_DAY;
-- CREATE TABLE ORDER_PER_DAY(PRODUCT_ID VARCHAR(50),
-- ORDER_DATE DATE,
-- QTY INT,
...
CREATE PROCEDURE UpdatePassSeller(IN PASSWD1 VARCHAR(50),IN ID VARCHAR(50))
BEGIN
	UPDATE SELLER SET PASSWD = PASSWD1 WHERE SELLER_ID = ID;
END //

DELIMITER ;

DELIMITER //
CREATE PROCEDURE POPULAR_PROD(IN BUY_DATE DATE)
BEGIN
    SELECT PRODUCT_ID,DATE_OF_PURCHASE,SUM(QTY) AS TOTAL_QTY FROM ORDERS GROUP BY DATE_OF_PURCHASE,PRODUCT_ID HAVING DATE_OF_PURCHASE = BUY_DATE ORDER BY TOTAL_QTY DESC;
END //
DELIMITER ;
